...
const assetList = [
    ...
    { name: 'phongTextFS', url: './shaders/phong.directionlit.fs.glsl', type: 'text' },
...
function updateAndRender() {
    ...
    if (appInput.left) { yaw -= rotationSpeed * time.deltaTime; }
    if (appInput.right) { yaw += rotationSpeed * time.deltaTime; }
    if (appInput.up) { pitch += rotationSpeed * time.deltaTime; }
    if (appInput.down) { pitch -= rotationSpeed * time.deltaTime; }

    pitch = Math.min(pitch, 89.0);
    pitch = Math.max(pitch, -89.0);

    const yawMatrix = new Matrix4().makeRotationY(yaw);
    const pitchMatrix = new Matrix4().makeRotationX(pitch);
    const lightRotationMatrix = yawMatrix.multiply(pitchMatrix);
    const rotatedLightDirection = lightRotationMatrix.multiplyVector(lightDirection);
    ...
    gl.uniform3f(uniforms.lightDirectionUniform, rotatedLightDirection.x, rotatedLightDirection.y, rotatedLightDirection.z);
