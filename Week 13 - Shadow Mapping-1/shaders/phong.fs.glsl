...
void main(void) {
  ...
  //vec3 shadowColor = texture2D(uShadowTexture, vTexCoords).rgb;
  ...
  // if (shadowColor.r < lightDepth - bias) {
  ...
  for (int y = -1; y <= 2; y++) {
    for (int x = -1; x <= 2; x++) {
      vec2 pcfOffset = vec2(float(x) - 0.5, float(y) - 0.5);
      vec2 pcfUV = lightSpaceUV + pcfOffset * uShadowTexelSize;
      float pcfDepth = texture2D(uShadowTexture, pcfUV).r;
      shadowFactor += step(lightDepth - bias, pcfDepth);
    }
  }

  shadowFactor /= 16.0;
  gl_FragColor = vec4(ambient + shadowFactor * (diffuseColor + specularColor), 1.0);
}
